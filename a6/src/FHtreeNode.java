public class FHtreeNode<E> implements Cloneable
{
   protected int mSize;
   protected KHTreeNodeNode<E> mRoot;
   
   public KHTreeNode() { clear(); }
   public boolean empty() { return (mSize == 0); }
   public int size() { return mSize; }
   public void clear() { mSize = 0; mRoot = null; }
   
   public KHTreeNodeNode<E> find(E x) { return find(mRoot, x, 0); }
   ...
   public KHTreeNodeNode<E> addChild( KHTreeNodeNode<E> treeNode,  E x )
   {
      ...
      if (mSize == 0)
      {
         ...
         mRoot = new KHTreeNodeNode<E>(x, null, null, null);
         mRoot.myRoot = mRoot;
         mSize = 1;
         return mRoot;
      }
      ...
      KHTreeNodeNode<E> newNode = new KHTreeNodeNode<E>(x, 
   ...
   public KHTreeNodeNode<E> find(KHTreeNodeNode<E> root, E x, int level)
   {
      KHTreeNodeNode<E> retval;
   ...
   public boolean remove(KHTreeNodeNode<E> root, E x)
   {
      KHTreeNodeNode<E> tn = null;
   ...
   private void removeNode(KHTreeNodeNode<E> nodeToDelete )
   {
   ...
   {
      KHTreeNode<E> newObject = (KHTreeNode<E>)super.clone();
   ...
   private KHTreeNodeNode<E> cloneSubtree(KHTreeNodeNode<E> root)
   {
      KHTreeNodeNode<E> newNode;
      if (root == null)
         return null;

      // does not set myRoot which must be done by caller
      newNode = new KHTreeNodeNode<E>
      (
         root.data, 
   ...
   private void setMyRoots(KHTreeNodeNode<E> treeNode)
   {
      if (treeNode == null)
         return;
   ...
   public void  display(KHTreeNodeNode<E> treeNode, int level) 
   {
      String indent;
   ...
   public <F extends Traverser<? super E>> 
   void traverse(F func, KHTreeNode<E> treeNode, int level)
   {
      if (treeNode == null)
         return;
